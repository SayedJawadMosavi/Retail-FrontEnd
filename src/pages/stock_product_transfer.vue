<template>
  <div>
    ...
    <BreadCrumbs
      ...
      page="ګودامونو ته د محصولاتو لیږد لست"
      create-text=" ګودامونو ته لیږدول"
      edit-text="ایدیت  "
      icon="mdi-shopping-outline"
      :show-create="scope(['stock_product_transfer_create'])"
      :show-delete="scope(['not_stock_product_transfer_delete'])"
      :show-restore="scope(['not_stock_product_transfer_restore'])"
      :show-force-delete="scope(['not_stock_product_transfer_force_delete'])"
    ...
    >
      <template #default>
        <VBtn class="font-weight-bold bg-info" to="stock">
          ګدام ته زیاتول او لست
...
const fetchRecord = async () => {
  try {
    ...
    const { data } = await axios.get("product_stocks_transfer", {
...
const deleteRecord = async (type = "delete") => {
  try {
    ...
    if (type == "force-delete")
      await axios.delete("force-delete-product_stocks_transfer/" + ids);
    datatableRefs.value.selectedItems = [];
    fetchRecord();
  } catch (error) {
    console.error(error);
