<template>
  <div>
    ...
    <BreadCrumbs
      ...
      page="له ګودام څخه ګودام ته د لیږد لست"
      create-text=" انتقال   "
      icon="mdi-shopping-outline"
      :show-create="scope(['stock_to_stock_transfer_create'])"
      :show-delete="scope(['not_stock_to_stock_transfer_delete'])"
      :show-restore="scope(['not_stock_to_stock_transfer_restore'])"
      :show-force-delete="scope(['not_stock_to_stock_transfer_force_delete'])"
...
const fetchRecord = async () => {
  try {
    ...
    const { data } = await axios.get("stock_to_stocks_transfer", {
...
const deleteRecord = async (type = "delete") => {
  try {
    ...
    if (type == "force-delete")
      await axios.delete("force-delete-stock_to_stocks_transfer/" + ids);
    datatableRefs.value.selectedItems = [];
    fetchRecord();
  } catch (error) {
    console.error(error);
