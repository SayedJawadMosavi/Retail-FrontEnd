<template>
  <div>
    ...
    />
    <ProductCategory ref="categoryRef" :fetch-record="fetchRecord" />
    ...
    >
      <template #status="{ item }">
        ...
        >
          <VTooltip activator="parent" location="top"> فعال کردن </VTooltip>
          <VIcon
            size="30"
        ...
        <VBtn v-else variant="text" icon @click="changeStatus(item)">
          <VTooltip activator="parent" location="top"> غیر فعال </VTooltip>
      ...
      <template #type="{ item }">
        <VChip
          small
          :color="item.type == 'income' ? 'info' : 'error'"
          class="font-weight-medium"
        >
          {{ item.type == "income" ? "آمد" : "رفت" }}
        </VChip>
      </template>
...
const changeStatus = async (item) => {
  try {
    ...
    const { data } = await axios.post(
      "expense-income-category-status/" + item.status + "/" + item.id
    );
